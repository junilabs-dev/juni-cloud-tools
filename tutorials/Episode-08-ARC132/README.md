...
2. **Enable Required APIs (Crucial for getting points):**
...
4. **Connect to the Compute Engine VM:**
...
5. In this new SSH terminal, run the following command, making sure to replace `YOUR_API_KEY_HERE` with the key you just copied:

```bash
export API_KEY="YOUR_API_KEY_HERE"
```

6. Once the `API_KEY` is set, download and run the automated script by pasting this command:
...
7. Wait for the script to say **"All tasks completed successfully!"**.
8. Go back to Qwiklabs and click **Check my progress** for all tasks.
